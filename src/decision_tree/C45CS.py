...
class ConditionNodeC45CS(ConditionNode):
    ...
    def split(self):
        if self.condition is None:
            ...
            for key, group in children_indices.items():
                self.children.update({key: ConditionNodeC45CS(parent=self, 
                                                            subset_indeces=set(group), 
                                                            splitted_attr_names=self.splitted_attr_names)})
            return self
    
class DecisionTreeC45CS(AbstractDecisionTree):
    def __init__(self, pos_class_weight: float = 50,
                        max_depth: int = 20,
                        min_samples_split = 2,
                        VERBOSE: bool = True):
    ...
    def fit(self, df_x: pd.DataFrame, df_y: pd.DataFrame):
        self.root = ConditionNodeC45CS(value=round(sum(df_y) / len(df_y)), 
    ...
    def __fit_rec(self, node: ConditionNodeC45CS, depth: int):
    ...
    def _print_tree_rec(self, node: ConditionNodeC45CS, depth):
