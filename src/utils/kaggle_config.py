...
def download_dataset(dataset_name="", path='datasets/', unzip=True):
    ...
    if dataset_name == "":
        if (not os.path.exists(kaggle_dataset + "/Iris.csv") and \
            not os.path.exists(kaggle_dataset + "/diabetes_prediction_dataset.csv") and \
            ...
            dataset_name_diabetes = os.getenv("KAGGLE_DATASET_LINK_DIABETES")
            ...
            kaggle_api.dataset_download_files(dataset_name_diabetes, path=path, unzip=unzip)
    else:
        match dataset_name:
            case "uciml/iris":
                if not os.path.exists(kaggle_dataset + "/Iris.csv"):
                    dataset_name_iris = os.getenv("KAGGLE_DATASET_LINK_IRIS")
                    kaggle_api.dataset_download_files(dataset_name_iris, path=path, unzip=unzip)
            case "iammustafatz/diabetes-prediction-dataset":
                if not os.path.exists(kaggle_dataset + "/diabetes_prediction_dataset.csv"):
                    dataset_name_diabetes = os.getenv("KAGGLE_DATASET_LINK_DIABETES")
                    kaggle_api.dataset_download_files(dataset_name_diabetes, path=path, unzip=unzip)
